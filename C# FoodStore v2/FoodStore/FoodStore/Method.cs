...
namespace FoodStore
{
    ...
    {//server: segir til um hvar serverinn er hýstur, hjá okkur er það phpmyadmin hjá tsuts.tskoli.is
        ...
        // --------------------------------------------------------------------------------------------------------------------
        // Method BuaTilLagerTypeID endar
        //---------------------------------------------------------------------------------------------------------------------
        // --------------------------------------------------------------------------------------------------------------------
        // Method KassiFinnaVoru 
        //---------------------------------------------------------------------------------------------------------------------
        public string[] KassiFinnaVoru(int ID)
        {
            string[] gogn = new string[2];
            if (OpenConnection() == true)
            {
                fyrirspurn = "SELECT productname, productprice from lager where ID='" + ID + "'";
                nySQLskipun = new MySqlCommand(fyrirspurn, sqltenging);
                sqllesari = nySQLskipun.ExecuteReader();
                while (sqllesari.Read())
                {
        ...
        // --------------------------------------------------------------------------------------------------------------------
        // Method KassiFinnaVoru Endar
        //---------------------------------------------------------------------------------------------------------------------
        // --------------------------------------------------------------------------------------------------------------------
        // Method KassiFinnaVerdSemInt
        //---------------------------------------------------------------------------------------------------------------------
        public int KassiFinnaVerdSemInt(int ID)
        {
            int verd = 0;
            if (OpenConnection() == true)
            {
                fyrirspurn = "Select productprice from lager where ID='" + ID + "'";
                nySQLskipun = new MySqlCommand(fyrirspurn, sqltenging);
                sqllesari = nySQLskipun.ExecuteReader();
                while (sqllesari.Read())
                {
                    verd = Convert.ToInt32(sqllesari.GetValue(0));
                }
                sqllesari.Close();
                CloseConnection();
                return verd;
